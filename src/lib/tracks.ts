...
// Fetch tracks based on search and genre filters
export async function fetchSearch({ q, genre }: SearchParams): Promise<Track[]> {
...
// Fetch a single track by its ID for the TrackDetailsPage
...
// Fetch related tracks by doing a broader search for the same genre
export async function relatedTracks (track: Track, limit = 4): Promise<Track[]>{
  try {
    // Fetch tracks of thte same genre
    const response = await fetch(`${API_BASE_URL}/search?genre=${encodeURIComponent(track.genre)}`)
    if (!response.ok) return [];
    const tracks: Track[] = await response.json();

    // Filter out the currenct track and limit the results
    return tracks.filter(t => t.id !== track.id).slice(0, limit);
  } catch (error) {
